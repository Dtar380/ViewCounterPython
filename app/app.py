...
def check_url(user: str, repo: str = None) -> list:
    ...
    if user and requests.get(f"{GITHUB}/{user}").status_code != 200:
        return [False, "User does not exist"]
    if repo and requests.get(f"{GITHUB}/{user}/{repo}").status_code != 200:
...
def give_views_url(user: str, repo: str):
    ...
    if request.args.get("onlyRead") == "True":
        ...
        # Returns SVG badge
        svg = requests.get(url=retrieve_url(request.args, user, repo)).text
...
def get_profile_views(user: str):
    ...
    if request.args.get("onlyRead") == "True":
        ...
        # Returns SVG badge
        svg = requests.get(url=retrieve_url(request.args, user)).text
