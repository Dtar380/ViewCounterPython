...
session.cache_disabled = True # Disable cache
...
def check_url(user: str, repo: str = None) -> list:
    ...
    if user and session.get(f"{GITHUB}/{user}").status_code != 200:
        return [False, "User does not exist"]
    if repo and session.get(f"{GITHUB}/{user}/{repo}").status_code != 200:
...
#####  APP FUNCTIONS  #####
# Function to delete the cache from the flask server
@app.after_request
...
def give_views_url(user: str, repo: str):
    ...
    if request.args.get("onlyRead") == "True":
        ...
        # Returns SVG badge
        svg = session.get(url=retrieve_url(request.args, user, repo)).text
...
def get_profile_views(user: str):
    ...
    if request.args.get("onlyRead") == "True":
        ...
        # Returns SVG badge
        svg = session.get(url=retrieve_url(request.args, user)).text
        return app.response_class(svg, mimetype="image/svg+xml")
